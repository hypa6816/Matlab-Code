function Design1Code(inputfile,outputfile)
...
%              if 3*numjoints - numreact ~= numbars
...
%              end
        ...
        case 2 % read coordinates of joints
            ...
            joints(jointid,:)=tmp(2:4)*0.0254*4;
...
function [barforces,reacforces] = force_analysis_3d(joints,connectivity,...
...
weightjoint = .0084; 
densitybar = .00114; 
joint_sums = ones(numjoints,1)*weightjoint;


% build Amat - loop over all joints
member_lengths = zeros(numbars,1);
current_member = 1;
for i=1:numjoints
   ...
   for ib=1:length(ibar)
       ...
       %compute bar masses
       full_length = sqrt((joint_j(1)-joint_i(1))^2+(joint_j(2)-joint_i(2))^2+(joint_j(3)-joint_i(3))^2);
       bar_length = full_length - magnetlength -diameterjoint;
       bar_weight = 1/2*bar_length*densitybar*9.807;
       member_lengths(current_member,1)=full_length;
       current_member = current_member+1;
       weight_sum = bar_weight + weightmagnet;
       joint_sums(i) = joint_sums(i) + weight_sum;
...
for i = 1:numjoints;
    bvec(3*i) = bvec(3*i)+joint_sums(i);
end
...
for is=1:numsamples 
    ...
    % compute forces in bars and reactions
    [barforces,reacforces] = force_analysis_3d(randjoints,connectivity,reacjoints,reacvecs,loadjoints,loadvecs);
